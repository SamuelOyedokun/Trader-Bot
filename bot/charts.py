...
def generate_sales_chart(rows, title="Sales Chart"):
    """Generate a bar chart from sales rows and return image bytes."""
    if not rows:
        return None
...
def send_image_via_twilio(phone, image_buf, caption, account_sid, auth_token, from_number):
    """Upload image to Twilio and send via WhatsApp."""
    try:
        # Upload to Twilio MMS
        from twilio.rest import Client
        client = Client(account_sid, auth_token)

        # Save image temporarily
        tmp_path = f"/tmp/chart_{phone}.png"
        with open(tmp_path, "wb") as f:
            f.write(image_buf.read())

        # Use Twilio to send image
        # For sandbox, we use a publicly hosted image
        # We'll use a free image hosting service
        import base64
        image_buf.seek(0)
        img_data = base64.b64encode(image_buf.read()).decode()
