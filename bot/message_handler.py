...
def handle_message(phone: str, text: str):
    ...
    try:
        ...
        # ─── VIEW STOCK ──────────────────────────────────────
            ...
            for s in stock:
                qty = s["quantity"]
                retail_qty = s.get("retail_quantity", 0)
                units_per_bulk = s.get("units_per_bulk", 1) or 1
                bulk_unit = s.get("bulk_unit", "bag")
                retail_unit = s.get("retail_unit", "unit")
                low_stock = "⚠️ LOW" if qty <= 2 else "✅"

                if retail_qty and units_per_bulk > 1:
                    lines.append(
                        f"{low_stock} {s['item']}: {qty:.1f} {bulk_unit}s "
                        f"/ {retail_qty:.0f} {retail_unit}s @ ₦{s['cost_price']:,.0f}"
                    )
                else:
                    lines.append(
                        f"{low_stock} {s['item']}: {qty} units @ ₦{s['cost_price']:,.0f}"
                    )
            send_whatsapp_message(phone, "\n".join(lines))
        ...
        # ─── RECORD PAYMENT ──────────────────────────────────
            ...
            remaining, pay_status = record_payment(phone, customer, amount)
        ...
        # ─── SET UNIT CONVERSION ─────────────────────────────
            ...
            if parsed.get("items") and parsed["items"][0].get("item"):
                ...
                retail_words = ["mudu", "paint", "rubber", "tin", "cup", "piece", "pieces", "wrap", "wraps", "unit", "units"]
                for word in words:
        ...
        elif intent == "view_unit_conversions":
            conversions = get_all_unit_conversions(phone)
            if not conversions:
                send_whatsapp_message(phone,
                    "No unit conversions set yet.\n\n"
                    "Try: '1 bag rice = 33 mudu'"
                )
                return
            lines = ["📐 *Your Unit Conversions*\n"]
            for c in conversions:
                lines.append(
                    f"📦 {c['item']}: 1 {c['bulk_unit']} = {c['units_per_bulk']:.0f} {c['retail_unit']}"
                )
            send_whatsapp_message(phone, "\n".join(lines))
        ...
        # ─── VIEW SECTION STOCK ───────────────────────────────
            ...
            for s in stock:
                qty = s["quantity"]
                low_stock = "⚠️ LOW" if qty <= 2 else "✅"
                lines.append(f"{low_stock} {s['item']}: {qty} units @ ₦{s['cost_price']:,.0f}")
            send_whatsapp_message(phone, "\n".join(lines))
        ...
        traceback.print_exc()
        try:
            send_whatsapp_message(phone, "⚠️ Something went wrong. Please try again.")
        except:
            pass